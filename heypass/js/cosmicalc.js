...
var part_func = function(i, part){
	...
	if(Parts_data[part][this.id]) throw "ID重複";
	pd = Parts_data[part][this.id] = setDefault(this, part, cmn_stat);

	if(part!="fixed"){
		if(Parts_data[pd.name]) throw "名称重複";
		...
		if(Parts_data.fixed[pd.name]) throw "名称重複";
...
$(".parts_list:not('#LIST_BD')").click(function(e){
	...
	if(pd.jointPart){
		if(ccd.parts_ref != Parts_none && ccd.parts_ref != pd && !ccd.parts_ref.jointPart){
			if(confirm(pd.name+" をこの位置に挿入しますがよろしいですか？\n「キャンセル」を選択した場合、現在のパーツと付け替えになります。")){
...
$("#TIO_OUT").click(function(){
	...
	str += "\n【構成チェック】\n";
	str += "COST： "+(Result.capa < Result.cost ? "コストオーバー" : "OK")+"\n";

	$.each(["BS","LG"], function(i, p){
		str += p+"： "+(Result[p.toLowerCase()] ? "OK" : "装着されていません。")+"\n";
	});
...
$("#TIO_READ").click(function(){
	if(Prop.changed){
		if(!confirm("構成が変更されています。\nこのまま進むと上書きされますがよろしいですか？")) return false;
		Prop.changed = false;
	}
	var str = $("#TIO_TEXTAREA")[0].value,
		parts = str.match(/【パーツ構成】[^【]+/)+"",
		cartridge = (str.match(/【カートリッジ】[^【]+/)+"").split("\n"),
		ccd = {part:[],cartridge:[]}, i = 0, tmp;

	if(parts!="null"){
		ccd.name = (str.match(/【ロボ名】[^【]+/)+"").split("\n")[1] || "";
		...
		parts.replace(/(BD|WB|LG|HD|BS|AM|WP|HDAC|FCAC)[:：][ 　\t]*([^\r\n]+)/g, function(str, p1, p2){
			...
			$.each(p2.split(/[ 　\t]+/), function(){
				if(this[0] == "["){
					...
					if(Master.tuneup[tn] == undefined) Message.set(tn+" はチューンデータが見つかりません。", 1);
		...
		$.each(cartridge, function(i){
			if(i){
				...
				if(str != ""){
					...
					else Message.set(str+" はカートリッジデータが見つかりません。", 1);
				}
			}
		});
	}else{
	...
	if(ccd.part.length){
		autoSetup(ccd);
		Message.set("構成を読込みました。");
		$(this).parents(".transparent_field").hide();
	}else{
	 Message.set("構成の読み込みに失敗しました。", 1);
	}
	Prop.changed = false;
});
...
$("#SS_SAVE").click(function(){
	...
	if(Assembly.bd.sb.ccd.parts_ref == Parts_none){
		if(ck){
			if(confirm("slot"+save_slot+" の保存データを消去します。よろしいですか？")){
...
$.each(["TEC","STR"], function(i, s){
	var str='<tr><th class="th1"><div class="right">'+s+'</div><div class="left">威力</div></th>',
	...
	try{
		...
		function check_str(obj){
			if(!str.match(obj ? /^[a-z]\w+:(?:-?\d+|"[^\"\n]*")$/ : /^(-?\d+|"[^\"\n]*")$/))
				errmsg.push("　　'"+str+"' は解析できませんでした。");
		};

		try{
			$.each(data, function(i, c){
				if(i > MAX_DATA_SIZE) throw "データが大きすぎるため解析を終了します。";
				if(fstr){
					 str += c;
					 if(c=='"') fstr = 0;
				}else{
					...
					case '\n':
						if(errmsg.length){
							errmsg.unshift('JSONパースエラー!　'+fname+'.json　'+line+'行目');
							errcnt += errmsg.length-1;
							if(errcnt > MAX_ERROR_COUNT) throw "エラー数が多すぎるため解析を終了します。";
					...
					case '}':
						...
						break;
					case '[':
						if(bc != ':') errmsg.push('　　"'+(str || bc)+c+'" は解析できませんでした。');
						brackets.push(0);
						b2++;
						str = "";
						bc = c;
	...
	this.exec = function(callback){
		...
		for(tcount=0,i=0; i<task.length; i++) (function(fname, cb){
			var webAccess = INI.webAccess && INI.accessData[fname],
				dataError = function(){
					Message.set('エラー!　'+fname+'　を読み込めませんでした。', 2);
			...
			expandData = function(data){
				if(data = evalJSON(secureJSON(data), fname)){
					Message.set(fname+" を取得しました。");
					$.each(data, function(i){
						try{
							cb.call(this, i, fname.indexOf("wp")>0 ? "wp" : fname);
						}catch(e){
							Message.set('警告!　file:'+fname+'.json　line:'+i+'　"'+this.name+'"は '+e+'エラーのため読み込めませんでした。', 1);
						}
					});
					Message.set("　展開完了。");
				}
			...
			getLocal = function(){
				if(webAccess){
					Message.set(fname+" を取得できませんでした。ローカルデータを取得します。", 1);
					$.ajax({
...
function setCookie(key, val, tmp, ck) {
	...
	if(document.cookie.length + tmp.length - ck.length > 4096){
		alert('Cookieの保存可能サイズをオーバーしています。\n他のセーブデータを削除するか、テキスト出力を行ってください。');
		return false;
	}
...
function setDefault(obj, part, cmn, ss){
	...
	if(obj.type){
		var typ = "";
		if(part=="bd"){
			if(obj.type.match(/[陸空砲補]/)) obj.type = RegExp.lastMatch;
			else obj.type = {l:"陸",a:"空",c:"砲",s:"補"}[obj.type] || "－";
		}else{
...
function autoSetup(ccd){
	...
	$.each(ccd.part, function(i){
		...
		if(elm.length) sb = elm[0].ccd_sb;
			...
			Message.set(this.part.toUpperCase()+'に"'+this.name+'"を装着できませんでした。', 1);
			return;
		}
	...
	if(Result.lv < j) Message.set("読み込めないカートリッジが存在します。", 1);
	$("#CURRENT_LV").text(Result.lv+"/10");
